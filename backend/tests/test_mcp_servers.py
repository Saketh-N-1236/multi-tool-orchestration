...
async def test_vector_search_server():
    ...
    async with MCPClient() as client:
        ...
        try:
            ...
            print(f"   [WARNING] Make sure Vector Search server is running on port {settings.vector_mcp_port}")
            return
        ...
        try:
            tools_info = await client.list_tools(server_url)
            print(f"   [OK] Server: {tools_info.get('server_name')}")
            print(f"   [OK] Tools found: {len(tools_info.get('tools', []))}")
            for tool in tools_info.get('tools', []):
                print(f"      - {tool['name']} (v{tool.get('tool_version', 'N/A')})")
        except Exception as e:
        ...
        try:
            result = await client.call_tool(
                ...
                params={
                    "collection": "test",
                    "documents": [
                        {"id": "1", "text": "Machine learning is a subset of AI", "metadata": {"topic": "AI"}},
                        {"id": "2", "text": "Python is a programming language", "metadata": {"topic": "Programming"}},
                        {"id": "3", "text": "Vector databases store embeddings", "metadata": {"topic": "Database"}}
                    ]
                }
            )
            ...
            if normalized['status'] == 'success':
                print(f"   [OK] Documents added: {normalized['data'].get('added_count', 0)}")
        except Exception as e:
        ...
        try:
            result = await client.call_tool(
                ...
                params={"query": "artificial intelligence", "collection": "test", "top_k": 2}
            )
            normalized = normalize_result(result, "search_documents")
            print(f"   [OK] Status: {normalized['status']}")
            if normalized['status'] == 'success':
                data = normalized['data']
                print(f"   [OK] Results found: {data.get('count', 0)}")
                for i, res in enumerate(data.get('results', [])[:2], 1):
                    print(f"      {i}. Score: {res.get('score', 0):.3f} - {res.get('text', '')[:50]}...")
        except Exception as e:
...
async def main():
    ...
    await test_vector_search_server()
